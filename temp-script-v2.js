...
const RING_CIRC = 2 * Math.PI * 17; // Updated for r=17 (was 2 * Math.PI * 14)
...
function setDay(d) {
  ...
  // If sidebar is expanded, refresh the detail view
  if (selectedRoom && planLayout.classList.contains('room-selected')) {
...
function animateViewBox(target, duration) {
  ...
  function step(now) {
    ...
    // Smooth ease-in-out settle curve per contract
    const t = raw < 0.5 ? 2 * raw * raw : 1 - Math.pow(-2 * raw + 2, 2) / 2;

    const x = current.x + (target.x - current.x) * t;
    const y = current.y + (target.y - current.y) * t;
    const w = current.w + (target.w - current.w) * t;
    const h = current.h + (target.h - current.h) * t;
...
function panToRoom(el) {
  ...
  const vx = currentVB.x + currentVB.w / 2;
  const vy = currentVB.y + currentVB.h / 2;

  let targetX = currentVB.x + (cx - vx) * 0.6;
  let targetY = currentVB.y + (cy - vy) * 0.6;

  // Clamp
  if (targetX < 0) targetX = 0;
  if (targetY < 0) targetY = 0;
  if (targetX + currentVB.w > FULL_VIEWBOX.w) targetX = FULL_VIEWBOX.w - currentVB.w;
  if (targetY + currentVB.h > FULL_VIEWBOX.h) targetY = FULL_VIEWBOX.h - currentVB.h;

  animateViewBox({ x: targetX, y: targetY, w: currentVB.w, h: currentVB.h }, 350);
}

// ══════════════════════════════════════════════════════════════════════════════
// Room Detail Render (full checklist, replaces both compact and overlay versions)
// ══════════════════════════════════════════════════════════════════════════════
function renderRoomDetailCompact(roomId) {
  // Updated: use same status token values
  const data = getRoomDay(roomId, currentDay);
  if (!data) {
...
function bindCheckboxes(roomId) {
  detailContent.querySelectorAll('.check-box').forEach(box => {
    box.onclick = (e) => {
      e.stopPropagation();
...
function selectRoom(el) {
  ...
  // Add room-selected class to layout for sidebar expansion
...
function clearSelection(silent = false) {
  ...
  // Remove room-selected class for sidebar collapse
  planLayout.classList.remove('room-selected');

  // Remove dimming from all rooms
  document.querySelectorAll('.room').forEach(room => {
